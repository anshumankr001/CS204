...
vector<string> inToPost(string str)
{
    ...
    f(i, 0, x)
    {
        ...
        if( (str[i] >= '0' && str[i] <= '9') || ((isOperator(str[i-1]) || str[i-1] == '(') && str[i] == '-') || (i == 0 && str[i] == '-') ) t += str[i];
        else
        {
            if(!t.empty()) s.pb(t);
            t.clear();
...
string unary(string s)
{
    ...
    {
        if(s[i] == '-' && s[i+1] == '-') p[j]++;
        else if(p[j] != 0 && p[j]%2 == 0)
        {
            t += '-';
            j++;
        }
        else if(p[j] != 0 && p[j]%2 == 1)
        {
            t += '+';
            j++;
        }
